  ...
  fDetector(det), fParticle(0), fEkin(0.)
{
  ...
  fEdepGap = fEdepGap2 = 0.;
  fEdepShield = fEdepShield2 = 0.;
}
...
void Run::ParticleCount(G4String name, G4double Ekin, G4int iVol)
{
  ...
  if (iVol == 4) {
   std::map<G4String, ParticleData>::iterator it = fParticleDataMap4.find(name);
   if ( it == fParticleDataMap4.end()) {
...
void Run::AddEdep(G4double edep1, G4double edep2, G4double edep3, G4double edep4)
{ 
  fEdepTarget  += edep1;
  ...
  fEdepGap  += edep3;
  fEdepGap2 += edep3*edep3;
  fEdepShield  += edep4;
  fEdepShield2 += edep4*edep4;    
}
...
void Run::Merge(const G4Run* run)
{
  ...
  fEdepGap   += localRun->fEdepGap;  
  fEdepGap2  += localRun->fEdepGap2;
  fEdepShield   += localRun->fEdepShield;  
  fEdepShield2  += localRun->fEdepShield2;  
  ...
  //map: created particle in gap      
  std::map<G4String,ParticleData>::const_iterator itg;
  for (itg = localRun->fParticleDataMap3.begin(); 
       itg != localRun->fParticleDataMap3.end(); ++itg) {
    
    G4String name = itg->first;
    const ParticleData& localData = itg->second;   
  ...
  //map: created particle in shield      
  std::map<G4String,ParticleData>::const_iterator itsh;
  for (itsh = localRun->fParticleDataMap4.begin(); 
       itsh != localRun->fParticleDataMap4.end(); ++itsh) {
    
    G4String name = itsh->first;
    const ParticleData& localData = itsh->second;   
...
void Run::EndOfRun() 
{
  ...
  /*G4cout << "\n Detector : Length = " 
         ...
         << " Material = " 
         << fDetector->GetDetectorMaterial()->GetName();
  G4cout << "\n Gap : Length = " 
         << G4BestUnit(fDetector->GetGapLength(),"Length")
         << " Thickness = " 
         << G4BestUnit(fDetector->GetGapThickness(),"Length")  
         << " Material = " 
         << fDetector->GetGapMaterial()->GetName();
  G4cout << "\n Shield : Length = " 
         << G4BestUnit(fDetector->GetShieldLength(),"Length")
         << " Thickness = " 
  ...
  // compute mean Energy deposited and rms in gap
  //
  fEdepGap /= TotNbofEvents; fEdepGap2 /= TotNbofEvents;
  rmsEdep = fEdepGap2 - fEdepGap*fEdepGap;
  if (rmsEdep>0.) rmsEdep = std::sqrt(rmsEdep);
  else            rmsEdep = 0.;
  
  G4cout << " Mean energy deposit in gap, in time window = "
         << G4BestUnit(fEdepGap,"Energy") << ";  rms = "
         << G4BestUnit(rmsEdep,    "Energy") 
         << G4endl;

  // compute mean Energy deposited and rms in shield
  //
  fEdepShield /= TotNbofEvents; fEdepShield2 /= TotNbofEvents;
  rmsEdep = fEdepShield2 - fEdepShield*fEdepShield;
  if (rmsEdep>0.) rmsEdep = std::sqrt(rmsEdep);
  else            rmsEdep = 0.;
  
  G4cout << " Mean energy deposit in shield, in time window = "
...
*/
  ...
  // frequency of processes in gap
  //
  G4cout << "\n Process calls frequency in gap:" << G4endl;
  index = 0;
  ...
  // frequency of processes in shield
  //
  G4cout << "\n Process calls frequency in shield:" << G4endl;
  index = 0;
 ...
 // particles count in gap
 //
 G4cout << "\n List of generated particles in gap:" << G4endl;
     
 std::map<G4String,ParticleData>::iterator itg;               
 for (itg = fParticleDataMap3.begin(); itg != fParticleDataMap3.end(); itg++) { 
    G4String name = itg->first;
    ParticleData data = itg->second;
    G4int count = data.fCount;
    G4double eMean = data.fEmean/count;
    G4double eMin = data.fEmin;
    G4double eMax = data.fEmax;
